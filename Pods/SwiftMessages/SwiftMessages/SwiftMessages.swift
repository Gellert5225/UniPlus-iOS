...
@objc public protocol SwiftMessagesDelegate {
    func didTapOnMessageView()
}
 ...
 */
@objc open class SwiftMessages: NSObject,PresenterDelegate {
    weak var delegate: SwiftMessagesDelegate?
    func tappedOn() {
        self.delegate?.didTapOnMessageView()
    }
    ...
    */
     ...
     */
    open func show(_ config: Config, viewProvider: @escaping ViewProvider) {
        DispatchQueue.main.async { [weak self] in
            guard let strongSelf = self else { return }
            let view = viewProvider()
            strongSelf.show(config, view: view)
        }
    }
    ...
    /**
     ...
     - Parameter viewProvider: A block that returns the view to be displayed.
     */
    @objc (showViewProvider:)
    open func show(_ viewProvider: @escaping ViewProvider) {
        show(defaultConfig, viewProvider: viewProvider)
    }
    ...
    func enqueue(_ presenter: Presenter) {
...
extension SwiftMessages {
    ...
    @objc (showViewProvider:)
    public static func show(_ viewProvider: @escaping ViewProvider) {
        globalInstance.show(viewProvider)
    }
    
    public static func show(_ config: Config, viewProvider: @escaping ViewProvider) {
        globalInstance.show(config, viewProvider: viewProvider)
    }
    
    public static func show(_ view: UIView) {
        globalInstance.show(view)
    }

    public static func show(_ config: Config, view: UIView) {
        globalInstance.show(config, view: view)
    }
    ...
    public static func hide(_ id: String) {
        globalInstance.hide(id)
    }
