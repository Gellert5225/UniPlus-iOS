...
@objc protocol PresenterDelegate : class {
    func hide(_ presenter: Presenter)
    func panStarted(_ presenter: Presenter)
    func panEnded(_ presenter: Presenter)
    func tappedOn()
}

@objc class Presenter: NSObject, UIGestureRecognizerDelegate {
    ...
    init(config: SwiftMessages.Config, view: UIView, delegate: PresenterDelegate) {
        ...
        tapRecognizer.addTarget(self, action: #selector(Presenter.didTap(_:)))
    ...
    func show(_ completion: @escaping (_ completed: Bool) -> Void) throws {
    ...
    func install() {
        ...
        do {
            ...
            let top = topLayoutConstraint(maskingView, presentationContext: presentationContext)
            let bottom = bottomLayoutConstraint(maskingView, presentationContext: presentationContext)
                ...
                case .top:
                    ...
                    if !UIApplication.shared.isStatusBarHidden {
                        if let vc = presentationContext as? WindowViewController {
                            ...
                            if !vc.sm_isVisible(vc.navigationBar) {
                                top += adjustable.statusBarOffset
                            }
                        } else {
                            //top += adjustable.statusBarOffset
    ...
    func topLayoutConstraint(_ view: UIView, presentationContext: UIViewController) -> NSLayoutConstraint {
        if case .top = config.presentationStyle, let nav = presentationContext as? UINavigationController, nav.sm_isVisible(nav.navigationBar) {
    ...
    func bottomLayoutConstraint(_ view: UIView, presentationContext: UIViewController) -> NSLayoutConstraint {
        if case .bottom = config.presentationStyle, let tab = presentationContext as? UITabBarController, tab.sm_isVisible(tab.tabBar) {
    ...
    func showAnimation(_ completion: @escaping (_ completed: Bool) -> Void) {

        showViewAnimation(completion)
    ...
    func showViewAnimation(_ completion: @escaping (_ completed: Bool) -> Void) {
    ...
    func hide(_ completion: @escaping (_ completed: Bool) -> Void) {
        switch config.presentationStyle {
        case .top, .bottom:
    ...
    @objc func pan(_ pan: UIPanGestureRecognizer) {
        ...
        case .ended, .cancelled:
            if closeSpeed > 750.0 || closePercent > 0.33 {
                ...
                closePercent = 0.0
                panTranslationY = 0.0
                showViewAnimation({ (completed) in
                    self.delegate?.panEnded(self)
                })
            }
        default:
            break
