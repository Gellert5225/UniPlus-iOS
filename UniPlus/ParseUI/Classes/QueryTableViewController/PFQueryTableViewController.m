...
@interface PFQueryTableViewController () {
    ...
    //NSInteger _currentPage;    // The last page that was loaded// The count of objects from the last load.
...
- (BFTask<NSArray<__kindof PFObject *> *> *)loadObjects:(NSInteger)page clear:(BOOL)clear completionBlock:(void (^)(NSArray *foundObjects, NSError *error))block {
    ...
    if ([self.sortingOrder isEqualToString:@"Newest"]) {
        ...
        [q orderByDescending:@"upVotes"];
    } else if ([self.sortingOrder isEqualToString:@"Unsolved"]){
        [q whereKeyDoesNotExist:@"correctAnswer"];
    } else {
        [q orderByDescending:@"createdAt"];
    }
    if (self.lastObject) {
...
- (BFTask<NSArray<__kindof PFObject *> *> *)loadObjects {
    _clearing = YES;
    _pageToLoad = 0;
    return [self loadObjects:_pageToLoad clear:_clearing];
}
...
- (void)loadNextPage {
    _clearing = NO;
    _pageToLoad = _currentPage + 1;
    if (!self.loading) {
        [self loadObjects:(_pageToLoad) clear:_clearing];
        [self _refreshPaginationCell];
    }
}
...
- (void)_refreshPaginationCell {
    if ([self _shouldShowPaginationCell]) {
//        NSLog(@"index path for pagination: %@", [self _indexPathForPaginationCell]);
//        [self.tableView reloadRowsAtIndexPaths:@[ [self _indexPathForPaginationCell] ] withRowAnimation:UITableViewRowAnimationNone];
//        NSLog(@"refreshing pagination cell");
    }
}
